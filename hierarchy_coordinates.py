...
def node_weighted_condense(A, num_thresholds=8, exp_threshold_distribution=None):
    ...
    if num_thresholds == 1:
        ...
        if exp_threshold_distribution is None:
            ...
            thresholds = exponentially_distribute(exponent=exp_threshold_distribution,
                                                                dist_max=np.max(A),
                                                                dist_min=np.min(A),
...
def feedforwardness_iteration(nx_graph):
    """
    :return: g = sum of feedforwardness for a single Directed Acyclic graph, num_paths = sum total paths considered
    """
...
def graph_entropy(directed_acyclic_graphs, forward_entropy=False):
    ...
    B = matrix_normalize(nx.to_numpy_array(initial_dag), row_normalize=False)
...
def average_hierarchy_coordinates(A, num_thresholds=8, exp_threshold_distribution=None):
    ...
    :param exp_threshold_distribution:
    :return:
    """
    np_A = []
    ...
    if check_binary(np_A):
        num_thresholds = 1
    else:
        num_thresholds = num_thresholds

    o, f, t = 0, 0, 0
    condensed_graphs, original_graphs = node_weighted_condense(A=np_A, num_thresholds=num_thresholds, exp_threshold_distribution=exp_threshold_distribution)
